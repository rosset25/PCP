...
*/
    ...
    for(int i = 0 ;i<numHebras;i++) {
        hebras[i]=new Hebra(fecha, colaTareas, MaxMin);
        hebras[i].start();
    }
    
    obtenMayorDiferencia_ParaleloDeFichero (colaTareas, numHebras);
  ...
  public static void obtenMayorDiferencia_ParaleloDeFichero (LinkedBlockingQueue<Tarea> colaTareas, int numHebras) {
          File           archivo = null;
          FileReader     fr      = null;
          BufferedReader br      = null;
          ...
          try
          {
                  ...
                  while( ( linea = br.readLine() ) != null ) {
                          ...
                          colaTareas.put(new Tarea(codPueblo));
                         // System.out.println("meto tarea");

                  }
                  //System.out.println("meto tareas envenenadas");
                  
                  for(int i=0; i < numHebras; i++) {
                	  colaTareas.put(new Tarea(true));
                  }
          } catch (Exception e) {
                  e.printStackTrace();
          }finally{
...
class Tarea{
        ...
        public Tarea(int pueblo) {
                this.pueblo=pueblo;
                this.envenenada=false;
        }

        public Tarea(boolean envenenada) {
                this.pueblo=-1;
                this.envenenada=true;
        }

        public boolean esEnvenenada() {
                return envenenada;
        }
...
class Hebra extends Thread{
        ...
        LinkedBlockingQueue<Tarea> colaTareas;
        PuebloMaximaMinima MaxMin;
        String fecha;

        public Hebra(String fecha, LinkedBlockingQueue<Tarea> colaTareas, PuebloMaximaMinima MaxMin) {
                this.fecha=fecha;
        ...
        public void run() {
                ...
                while (!acaba) {
                        try {
                        	//System.out.println("espero a tarea");
                                tarea=colaTareas.take();
                              //  System.out.println("obtengo tarea");
                                
                        } catch (InterruptedException e) {
                                e.printStackTrace();
                        }
                        if(!tarea.esEnvenenada()) {

                                EjemploTemperaturaProvincia.ProcesaPueblo(fecha, tarea.getPueblo(), MaxMin,false);
